...
const MyBookings = () => {
  ...
  const [activeTab, setActiveTab] = useState('overview'); // overview, timeline, concierge, billing, review, support
  ...
  const fetchMyBookings = async () => {
    ...
    try {
      ...
      // Refresh current active booking details in the drawer if open
      if (activeBooking) {
  ...
  // Sync review input values with active booking
  useEffect(() => {
    if (activeBooking) {
      setSubmitRating(activeBooking.review?.rating || 5);
      setSubmitComment(activeBooking.review?.comment || '');
    } else {
      setSubmitRating(5);
      setSubmitComment('');
    }
  ...
  // Categorize Bookings (Ongoing, Upcoming, Past)
  const categorizeBookings = () => {
    ...
    bookings.forEach(booking => {
      ...
      if (booking.status === 'CANCELLED' || booking.status === 'REFUNDED' || booking.status === 'COMPLETED') {
        past.push(booking);
      } else if (today >= checkInDateOnly && today <= checkOutDateOnly) {
        ongoing.push(booking);
      } else if (today < checkInDateOnly) {
  ...
  const { upcoming, ongoing, past } = categorizeBookings();
    ...
    <div className="max-w-6xl mx-auto px-4 py-12">
      ...
      {bookings.length === 0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {/* Main Bookings Grid (2/3 width) divided into 3 sections */}
          <div className="lg:col-span-2 space-y-12">
            ...
            {ongoing.length > 0 && (
              <div>
                ...
                <div className="space-y-6">
                  {ongoing.map(booking => renderBookingCard(booking))}
                </div>
              </div>
            )}
            ...
            <div>
              ...
              {upcoming.length === 0 ? (
                <div className="bg-gray-50/50 rounded-2xl p-8 text-center border border-dashed border-gray-200 text-xs text-gray-500 font-medium leading-relaxed">
                  No upcoming reservations planned.
                </div>
              ) : (
                <div className="space-y-6">
            ...
            <div>
              ...
              {past.length === 0 ? (
                ...
                <div className="space-y-6">
                  {past.map(booking => renderBookingCard(booking))}
                </div>
              )}
            </div>
      ...
      {activeBooking && (() => {
        // Calculate next day of check-in for review activation
        const checkInDate = new Date(activeBooking.checkIn);
        const checkInDateOnly = new Date(checkInDate.getFullYear(), checkInDate.getMonth(), checkInDate.getDate());
        const dayAfterCheckIn = new Date(checkInDateOnly.getTime() + 24 * 60 * 60 * 1000);
        const isReviewAllowed = new Date() >= dayAfterCheckIn;
            ...
            <div className="fixed inset-y-0 right-0 z-[120] w-full max-w-xl bg-white shadow-2xl overflow-y-auto flex flex-col h-full animate-slide-in">
              ...
              <div className="border-b border-gray-100 px-6 flex gap-4 overflow-x-auto whitespace-nowrap scrollbar-none">
                {[
                  ...
                  { id: 'review', label: 'Review Stay', icon: <Award size={14} /> },
              ...
              <div className="p-6 flex-1 overflow-y-auto space-y-6">
                ...
                {/* Tab 5: RATING & REVIEW (Activated next day of check-in) */}
                {activeTab === 'review' && (
                  <div className="space-y-6">
                    ...
                    {activeBooking.status === 'CANCELLED' || activeBooking.status === 'REFUNDED' ? (
                      <div className="p-4 bg-rose-50 text-rose-800 text-xs rounded-2xl border border-rose-100 flex items-start gap-2.5 leading-relaxed">
                        <AlertCircle size={16} className="shrink-0 mt-0.5 text-rose-600" />
                        <span>Reviews are only available for confirmed stays. This booking has been cancelled.</span>
                      </div>
                      ...
                      <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 space-y-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <span 
                                key={star} 
                                className={`text-xl ${star <= activeBooking.review.rating ? 'text-amber-400' : 'text-gray-200'}`}
                              >
                                ★
                              </span>
                            ))}
                        ...
                        <div>
                          <span className="text-[10px] text-gray-400 font-bold uppercase tracking-wider block">Your Feedback</span>
                          <p className="text-gray-700 text-xs mt-1.5 italic leading-relaxed bg-white border border-gray-100 p-4 rounded-xl">
                            "{activeBooking.review.comment || 'No description provided.'}"
                          </p>
                        </div>
                      </div>
                    ) : !isReviewAllowed ? (
                      <div className="p-5 bg-amber-50 text-amber-800 text-xs rounded-2xl border border-amber-100 space-y-4">
                        <div className="flex items-start gap-2.5 leading-relaxed">
                          <Clock size={20} className="shrink-0 text-amber-600 mt-0.5 animate-pulse" />
                          <div>
                            <p className="font-bold text-amber-950">Review Option Locked</p>
                            <p className="mt-0.5 text-amber-900/95">
                              This option will activate automatically on the next day of your check-in.
                            </p>
                          </div>
                        </div>
                        <div className="border-t border-amber-200/50 pt-3 text-[10px] font-bold text-amber-700 uppercase tracking-wider">
                          Unlocks on: {dayAfterCheckIn.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' })}
                        </div>
                      </div>
                    ) : (
                      <form onSubmit={handleSubmitReview} className="space-y-5">
                        <div className="space-y-2">
                          <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Your Rating</label>
                          <div className="flex gap-2">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <button
                                ...
                                className={`text-4xl transition-transform hover:scale-110 ${star <= submitRating ? 'text-amber-400' : 'text-gray-200'}`}
                              >
                                ★
                              </button>
                            ))}
                        ...
                        <div className="space-y-2">
                          <label className="block text-xs font-bold text-gray-700 uppercase tracking-wider">Write Review Comments</label>
                          <textarea
                            rows={4}
                            value={submitComment}
                            onChange={(e) => setSubmitComment(e.target.value)}
                            placeholder="Share your stay experience, room comfort, hospitality service, or amenity details..."
                            className="w-full text-xs p-3.5 border border-gray-200 rounded-xl focus:border-brand-500 focus:ring-1 focus:ring-brand-500 outline-none resize-none leading-relaxed"
                            maxLength={1000}
                          />
                        </div>
                        ...
                        <button
                          ...
                          className="w-full bg-brand-600 text-white font-bold py-3.5 rounded-xl text-xs hover:bg-brand-700 transition-colors shadow-md shadow-brand-500/10 flex items-center justify-center gap-1.5"
                        >
                          {submittingReview ? <RefreshCw size={14} className="animate-spin" /> : <CheckCircle2 size={14} />}
